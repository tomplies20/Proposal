...
def SVD(chiral_order, SVD_order, lamb):
    V00_sum = 0
    V01_sum = 0
    V10_sum = 0
    V11_sum = 0
    singular_values_path = './potentials/SVD_files/singular_values/'
    operators_path = './potentials/SVD_files/operators/'

    sv00 = singular_values_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_singular_values" % (
        orders[chiral_order], lamb, S[0], L[0], Lprime[0], J[0], T[0])
    sv01 = singular_values_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_singular_values" % (
        orders[chiral_order], lamb, S[1], L[1], Lprime[1], J[1], T[1])
    sv10 = singular_values_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_singular_values" % (
        orders[chiral_order], lamb, S[2], L[2], Lprime[2], J[2], T[2])
    sv11 = singular_values_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_singular_values" % (
        orders[chiral_order], lamb, S[3], L[3], Lprime[3], J[3], T[3])

    singular_values00 = np.loadtxt(sv00)
    singular_values01 = np.loadtxt(sv01)
    singular_values10 = np.loadtxt(sv10)
    singular_values11 = np.loadtxt(sv11)

    for o in range(SVD_order + 1):
        sv00_ = singular_values00[o]
        sv01_ = singular_values01[o]
        sv10_ = singular_values10[o]
        sv11_ = singular_values11[o]
        '''
        sv00_ = sv00_ + sv00_ * percent[0, o]
        sv01_ = sv01_ + sv01_ * percent[1, o]
        sv10_ = sv10_ + sv10_ * percent[2, o]
        sv11_ = sv11_ + sv11_ * percent[3, o]
        '''
        V00_new = 0
        V01_new = 0
        V10_new = 0
        ...
        file00 = operators_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_SVD_order_%s" % (
        orders[chiral_order], lamb, S[0], L[0], Lprime[0], J[0], T[0], o + 1)
        file01 = operators_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_SVD_order_%s" % (
        orders[chiral_order], lamb, S[1], L[1], Lprime[1], J[1], T[1], o + 1)
        file10 = operators_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_SVD_order_%s" % (
        orders[chiral_order], lamb, S[2], L[2], Lprime[2], J[2], T[2], o + 1)
        file11 = operators_path + "SVD_chiral_order_%s_lambda_%s_SLLJT_%s%s%s%s%s_SVD_order_%s" % (
        orders[chiral_order], lamb, S[3], L[3], Lprime[3], J[3], T[3], o + 1)
        [V00, Vmat_1, Tkin_1, mesh_points, mesh_weights] = read_Vchiral(file00, Nrows)
        [V01, Vmat_1, Tkin_1, mesh_points, mesh_weights] = read_Vchiral(file01, Nrows)
        [V10, Vmat_1, Tkin_1, mesh_points, mesh_weights] = read_Vchiral(file10, Nrows)
        [V11, Vmat_1, Tkin_1, mesh_points, mesh_weights] = read_Vchiral(file11, Nrows)

        V00_new = np.copy(V00) * sv00_
        V01_new = np.copy(V01) * sv01_
        V10_new = np.copy(V10) * sv10_
        V11_new = np.copy(V11) * sv11_
    ...
    K = compute_K_matrix_coupled(V00_sum, V01_sum, V10_sum, V11_sum, Nrows, 6)
...
LO_phaseshifts = SVD(0, 4, "2.00")
NLO_phaseshifts = SVD(1, 4, "2.00")
N2LO_phaseshifts = SVD(2, 4, "2.00")
N3LO_phaseshifts = SVD(3, 4, "2.00")
N4LO_phaseshifts = SVD(4, 4, "2.00")
