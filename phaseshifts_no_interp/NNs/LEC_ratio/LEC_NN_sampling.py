...
def ratio_test(ratio, phaseshift_set_):
      # default 0.68
    ...
    lin_space = np.linspace(1, 0, grid_size)
    ...
    for m in range(new_points):
        ...
        weights[m] = weight/grid_size
        print('avg weight: ' + str(weight/grid_size))
        if weight/grid_size >= 0.1: #0.05
...
LEC_set_ratio_NN, phaseshifts_NN, weights_NN = ratio_test(ratio_NN, phaseshift_set_)
...
LEC_set_final, phaseshift_set_final_interpolated, weights = ratio_test(ratio, phaseshift_set_final_non_uniform_interpolation)
